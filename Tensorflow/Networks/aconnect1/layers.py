    ...
    def build(self,input_shape):
        ...
        if(self.Wstd != 0 or self.Bstd != 0): #If the layer will take into account the standard deviation of the weights or the std of the bias or both
            if(self.Bstd != 0):
                ...
                self.infBerr = self.infBerr.numpy() #It is necessary to convert the tensor to a numpy array, because tensors are constant 
                                                    #and therefore cannot be changed. This was necessary to change the error matrix/array 
                                                    #when Monte Carlo was running.
            else:
                self.Berr = tf.constant(1,dtype=self.d_type)
            if(self.Wstd !=0):
