...
def model_creation(isAConnect=False,Wstd=0,Bstd=0,
        ...
        if(not(isAConnect)):
                model = tf.keras.models.Sequential([
                        ...
                        RandomTranslation(0.1,0.1),#RandomRotation(0.1),
                        RandomZoom(0.2),
                ...
                model = tf.keras.models.Sequential([
                        ...
                        ReLU(),
                        MaxPool2D(pool_size=(3,3),strides=(2,2),padding="SAME"),
                        Conv_AConnect(filters=512,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",Op=2,isQuant=isQuant,bw=bw),
                        BatchNormalization(),
                        ReLU(),
                        Conv_AConnect(filters=512,kernel_size=(3,3),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,pool=Conv_pool,padding="SAME",Op=2,isQuant=isQuant,bw=bw),
                        BatchNormalization(),
