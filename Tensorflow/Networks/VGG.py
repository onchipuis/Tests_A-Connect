...
from tf.keras.layers import InputLayer, Conv2D, Dense, MaxPool2D, Flatten
from tf.keras.layers import BatchNormalization, Dropout, ReLU, SoftMax
#import math
#import numpy
#import pathlib
		...
		model = tf.keras.models.Sequential([
			...
			#tf.keras.layers.experimental.preprocessing.Resizing(100,100),    
		        ...
		        BatchNormalization(),
                        ReLU(),
		        MaxPool2D(pool_size=(2,2), strides=(2,2),padding="SAME"),
		        Conv_AConnect(filters=128, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd, pool=4,strides=1,padding="SAME",Op=1,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
                        ReLU(),
		        Conv_AConnect(filters=128, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd, pool=4,strides=1,padding="SAME",Op=1,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
                        ReLU(),
		        MaxPool2D(pool_size=(2,2), strides=(2,2),padding="SAME"),
			Conv_AConnect(filters=256, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd,pool=4, strides=1,padding="SAME",Op=1,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
                        ReLU(),
		        Conv_AConnect(filters=256, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd,pool=4, strides=1,padding="SAME",Op=1,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
		        ...
		        Conv_AConnect(filters=512, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd, pool=4,strides=1,padding="SAME",Op=1,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
                        ReLU(),
		        Conv_AConnect(filters=512, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd,pool=4, strides=1,padding="SAME",Op=1,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
		        ...
		        BatchNormalization(),
                        ReLU(),
		        MaxPool2D(pool_size=(3,3), strides=(2,2),padding="SAME"),
		        Conv_AConnect(filters=512, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd,pool=4, strides=1,padding="SAME",Op=2,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
                        ReLU(),
		        Conv_AConnect(filters=512, kernel_size=(3,3), Wstd=Wstd,Bstd=Bstd, pool=4,strides=1,padding="SAME",Op=2,Slice=1,d_type=tf.dtypes.float16),
		        BatchNormalization(),
