...
from aconnect1.layers_only_forward import Conv_AConnect, FC_AConnect 
...
def model_creation(isAConnect=False,Wstd=0,Bstd=0,
        ...
        Conv_pool=2,FC_pool=2,errDistr="normal",**kwargs):
	...
	if(not(isAConnect)):
		...
		model = tf.keras.Sequential([
			InputLayer(input_shape=[32,32]),
			Reshape((32,32,1)),
			Conv_AConnect(64,kernel_size=(4,4),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,isQuant=isQuant,bw=bw,Op=2,strides=1,padding="VALID",pool=Conv_pool,d_type=tf.dtypes.float16),
                        BatchNormalization(),
			Activation('relu'),           
                        MaxPool2D(pool_size=(2,2),strides=(2,2),padding="valid"),
			Conv_AConnect(64,kernel_size=(4,4),Wstd=Wstd,Bstd=Bstd,errDistr=errDistr,isQuant=isQuant,bw=bw,Op=2,strides=1,padding="VALID",pool=Conv_pool,d_type=tf.dtypes.float16),
			...
			Activation('relu'),           
                        MaxPool2D(pool_size=(2,2),strides=(2,2),padding="valid"),
			Flatten(),
			FC_AConnect(256,Wstd,Bstd,errDistr=errDistr,isQuant=isQuant,bw=bw,pool=FC_pool,d_type=tf.dtypes.float16),
                        BatchNormalization(),           
			Activation('relu'),                       
			FC_AConnect(64,Wstd,Bstd,errDistr=errDistr,isQuant=isQuant,bw=bw,pool=FC_pool,d_type=tf.dtypes.float16),
                        BatchNormalization(),           
			Activation('relu'),                       
