...
def make_layer(x, planes, blocks, stride=1,**AConnect_args):
    ...
    x = basic_block(x, planes, stride, downsample,**AConnect_args)
    for i in range(1, blocks):
        x = basic_block(x, planes,**AConnect_args)
...
def resnet(input_shape, blocks_per_layer, num_classes=100,
    ...
    x = make_layer(x, 128, blocks_per_layer[1], padding="SAME",stride=2,pool=Conv_pool,**AConnect_args)
    x = make_layer(x, 256, blocks_per_layer[2], padding="SAME",stride=2,pool=Conv_pool,**AConnect_args)
    x = make_layer(x, 512, blocks_per_layer[3], padding="SAME",stride=2,pool=Conv_pool,**AConnect_args)
