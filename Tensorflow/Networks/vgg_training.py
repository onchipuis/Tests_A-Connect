...
if not(isAConnect):
    ...
    Nlayers = [1,4,8,11,15,18,21,25,28,31,35,38,41] #A-Connect layer numbers     
    ...
    for i in range(len(FC_pool_aux)):
        for j in range(len(Wstd_aux)):
            for k in range(len(errDistr)):
                ...
                if isAConnect[d]:
                    for p in range(len(Nlayers)):
                        model.layers[Nlayers[p]].set_weights(model_aux.layers[NlayersBase[p]].get_weights())
