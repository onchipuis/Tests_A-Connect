...
def LeNet5(Xtrain=None,Xtest=None,isAConnect=False,Wstd=0,Bstd=0,isBin="no"):
	...
	if(not(isAConnect)):
		model = tf.keras.Sequential([
			...
			Conv.Conv(6,kernel_size=(5,5),strides=(4,4),isBin=isBin),#tf.keras.layers.Conv2D(6,kernel_size=(5,5),strides=(1,1),padding="valid",activation="tanh"),
            tf.keras.layers.BatchNormalization(),            
			tf.keras.layers.Activation('relu'),            
            tf.keras.layers.AveragePooling2D(pool_size=(2,2),strides=(2,2),padding="same"),
			Conv.Conv(16,kernel_size=(5,5),strides=1,padding="same",isBin=isBin),#tf.keras.layers.Conv2D(16,kernel_size=(5,5),strides=(1,1),padding="valid",activation="tanh"),
            tf.keras.layers.BatchNormalization(),            
			tf.keras.layers.Activation('relu'),			
            tf.keras.layers.AveragePooling2D(pool_size=(2,2),strides=(2,2),padding="same"),
