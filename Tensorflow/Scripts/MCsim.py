...
def MCsim(net,Xtest,Ytest,M,Wstd,Bstd,force,Derr=0,net_name="Network",custom_objects=None,SRAMsz=[1024,1024],SRAMBsz=[1024],optimizer=tf.keras.optimizers.SGD(learning_rate=0.01,momentum=0.9),loss=['sparse_categorical_crossentropy'],metrics=['accuracy']):
...
#	global parallel
	...
	for i in range(M):
		[NetNoisy,Wstdn,Bstdn] = add_Wnoise.add_Wnoise(local_net,Wstd,Bstd,force,Derr,SRAMsz=SRAMsz,SRAMBsz=SRAMBsz)
...
#	print('Max. Accuracy: %.1f%%\n'% 100.0*np.amax(acc_noisy))
	...
	np.savetxt('./Results/'+net_name+'_simerr_'+str(100*Wstd)+'_'+str(100*Bstd)+'.txt',acc_noisy,fmt="%.2f")
