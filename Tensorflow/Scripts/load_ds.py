...
def load_ds(imgSize=[28,28], Quant=8):
	...
	if (imgSize != [28,28]):
		...
		x_train = tf.image.resize(x_train,[imgSize[0],imgSize[1]],method=tf.image.ResizeMethod.BICUBIC,antialias=True) #This function applies a resize similar to imresize in matlab
		#x_train = tf.cast(x_train,tf.uint8)
		x_test = tf.image.resize(x_test,[imgSize[0],imgSize[1]],method=tf.image.ResizeMethod.BICUBIC,antialias=True)
